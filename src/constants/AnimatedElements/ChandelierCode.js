...
class Dot {
  constructor(x, y, lightColor = '#ffcc00', ropeColor = '#aaa') {
...
class Rope {
  constructor(config) {
    this.ropeColor = config.ropeColor || '#aaa';
  ...
  create() {
    for (let i = 0; i < this.segments; i++) {
      this.dots.push(new Dot(this.x, this.y + i * this.gap, this.lightColor, this.ropeColor));
    }
    for (let i = 0; i < this.segments - 1; i++) {
      this.sticks.push(new Stick(this.dots[i], this.dots[i + 1], this.ropeColor));
    }
  }
...
const Chandelier = ({ ropeColor = '#aaa', lightColor = '#ffcc00' }) => {
  ...
  useEffect(() => {
    ...
    const createRopes = () => {
      ...
      for (let i = 0; i < totalRopes; i++) {
        ...
        const segments = 10;
        const rope = new Rope({ x, y, gap, segments, ropeColor, lightColor });
        rope.pin(0);
        ropes.push(rope);
      }
    };
